...
2. `verdict-cases.json` 的新用例按方向分别验收——**正向**用例（期望命中「中」档的那些）在**两端现有实现下必须全部失败**，若有一条意外通过，说明它没测到新东西；**反向**护栏用例（期望不命中／不贡献信号的那些）在空实现下**天然为绿，属预期**，不得因此把它们删掉——它们钉的是「新信号不得误命中」（ECS 缺失不算泄露、两个 STUN 不一致不算命中、srflx 跨协议族不算命中），要等两端探测接上后才承重。真正的红线由父计划全局验收标准 2 兜底：**故意翻转 O5 或 O6 的档位后两端同时变红**
