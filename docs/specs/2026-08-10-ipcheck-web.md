...
> **本节已迁出。** 判级规则现在住在 **[docs/verdict.md](../verdict.md)**——那是 normative 契约，
> 由 ipcheck Web 与 ipcheck CLI **共同**实现，并由 `docs/verdict-cases.json` 的 golden 向量
> 在两端同时验证（[ADR-0010](../adr/0010-verdict-contract-normative-cli-full-implementation.md)）。
>
> 同一套判级规则不能同时住在一份 normative 文档和一份 descriptive 设计规格里——
> 那正是 CLAUDE.md「Authoritative Source」要防的情形：两个对象互相打架时，
> 读者不知道该信哪一个。本节因此只留指路，不再自述规则。
>
> | 原小节 | 现在看哪里 |
> |---|---|
> | 3.1 综合结论三档 | [verdict.md §2 信号域](../verdict.md) 与 §3.1 |
> | 3.2 分项颜色 | [verdict.md §6](../verdict.md) |
> | 3.3 两段式 | [verdict.md §3.2 三形态](../verdict.md) |
> | 与 CLI 的差异 | [verdict.md §5 两端差异登记表](../verdict.md) |
...
2. **检测项卡片流** — 分为两个分区，各带小标题与一句说明：
   - 「网页可测到的」— O1, O2, O3, O4
   - 「需要 CLI 才能测」— C1…C5，均为灰卡

   两分区而非穿插：穿插版把「本站能做什么」与「本站结构性做不到什么」揉在一条卡片流里，用户读不出边界；灰卡集中后「网页 4 项 / CLI 补 5 项」的分工一眼可辨。

   一键复制 `pip install ai-ipcheck` 全站只出现一次，在落地内容的「安装 CLI」段；灰卡内与灰卡分区末尾都不再重复。
...
英语为默认语言与源语言，五个语种各自一份独立静态入口（ADR-0009）：

| 语种 | 路径 | `<html lang>` | 方向 | 译文状态 |
|---|---|---|---|---|
| English | `/`（别名 `/en`，canonical 指向 `/`） | `en` | ltr | 源语言 |
| 简体中文 | `/zh-hans` | `zh-Hans` | ltr | 已译全 |
| 繁體中文 | `/zh-hant` | `zh-Hant` | ltr | 待补译 |
| Русский | `/ru` | `ru` | ltr | 待补译 |
| العربية | `/ar` | `ar` | **rtl** | 待补译 |

- **语言完全由路径决定**，右上角语言菜单手动切换。**不做 `Accept-Language` 自动跳转**——本站用户大量在用代理，按地区/语言自动跳转正是他们最反感的行为，且会让「我看到的是哪个版本」变得不可预测。不写 localStorage / cookie，不持久化语言偏好。
- **未译字段按字段粒度回落英文源**，补译可以一条一条来。未注册的语种路径（`/zh`、`/fr`）是真实 404，不回退到任何语言。
- **RTL 与译文进度无关**：`/ar` 从第一天起就是 `dir="rtl"` 的镜像布局，CSS 用逻辑属性（`border-inline-start` / `text-align: start`）而非 `left` / `right`。
- 每个语种页声明全部 `hreflang` 备选与 `x-default`。
