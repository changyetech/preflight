...
export function usePanel() {
  ...
  const runGeo = useCallback(async () => {
    ...
    try {
      ...
      const failed = { status: "failed", reason: reasonOf(error) } as const;
  ...
  const runRisk = useCallback(async (turnstileToken: string) => {
    setPanel((prev) => ({ ...prev, o4: { status: "running" } }));

    try {
      const data = await fetchRisk(turnstileToken);
      setPanel((prev) => ({ ...prev, o4: { status: "done", data } }));
    } catch (error) {
      // proxycheck 不可用（5001）是检测失败，不是低风险——绝不能呈现成「没查出问题」。
      setPanel((prev) => ({
        ...prev,
        o4: { status: "failed", reason: reasonOf(error) },
      }));
    }
  }, []);
