...
//
// 分母恒为 10。规格 3.4 只列了「已完成 / 需 CLI / 检测失败」三档，但按需项 O4 在被触发前
// 既没完成、也没失败、更不是需 CLI——三档装不下它。故模型多留一档「按需未测」，
// 由 done + needCli + failed + pending ≡ 10 这条不变量兜底；pending 归零时三档之和自然恒为 10。
...
describe("computeCoverage", () => {
  it("四档之和恒为 10，需 CLI 恒为 4", () => {
    const panels: PanelState[] = [
      ...
      },
      {
        ...
        o5: { status: "failed", reason: "x" },
        o6: { status: "failed", reason: "x" },
      },
      { ...FIRST_SCREEN, o6: { status: "failed", reason: "webrtc disabled" } },
    ];
  ...
  it("首屏：已完成 5 · 需 CLI 4 · 检测失败 0 · 按需未测 1", () => {
  ...
  it("O5 无从比对仍记「已完成」，不是「检测失败」（契约 §2.5）", () => {
    // 探测成功了，只是回答里不含可判定的信息。记成失败会诱导用户反复刷新一个
    // 永远不会变的结果——不发 ECS 的 DNS 服务商刷新一万次也还是不发。
    ...
    expect(c).toEqual({ done: 5, needCli: 4, failed: 0, pending: 1 });
  });

  it("点击 O4 后已完成由 5 变 6，pending 归零，三档之和恒为 10（验收标准 5）", () => {
    const c = computeCoverage({
      ...FIRST_SCREEN,
      o4: {
        status: "done",
    ...
    expect(c).toEqual({ done: 6, needCli: 4, failed: 0, pending: 0 });
  ...
  it("配额耗尽计入「检测失败」而非「已完成」（规格 5.3）", () => {
    ...
    expect(c).toEqual({ done: 5, needCli: 4, failed: 1, pending: 0 });
  });

  it("检测中计入 pending，不预支为已完成", () => {
    expect(
      computeCoverage({ ...FIRST_SCREEN, o2: { status: "running" } }),
    ).toEqual({ done: 4, needCli: 4, failed: 0, pending: 2 });
  });
});
